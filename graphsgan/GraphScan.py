...
from graphsgan.functional import log_sum_exp, pull_away_term
from torch.utils.data import DataLoader, TensorDataset
import sys
from torch.nn.parameter import Parameter
import argparse
from graphsgan.network import Generator, Discriminator
import tensorboardX
import os
import random
from graphsgan.featureprocess import FeatureGraphDataset
import pickle as pkl
from graphsgan.data_loader import load_data
...
class GraphSGAN(object):
    ...
    def trainD(self, idf_label, y, idf_unlabel):
        x_label, x_unlabel, y = self.make_input(*idf_label), self.make_input(*idf_unlabel), Variable(y,
                                                                                                     requires_grad=False)
        if self.args.cuda:
            x_label, x_unlabel, y = x_label.cuda(), x_unlabel.cuda(), y.cuda()
        output_label, (mom_un, output_unlabel), output_fake = self.D(x_label, cuda=self.args.cuda), self.D(x_unlabel,
                                                                                                           cuda=self.args.cuda,
                                                                                                           feature=True), self.D(
            ...
            for batch_num in range(NUM_BATCH):
                ...
                if (batch_num + 1) % self.args.log_interval == 0:
                    ...
                    self.writer.add_scalars('loss', {'loss_supervised': ll, 'loss_unsupervised': lu, 'loss_gen': lg},
                                            gn)
                    self.writer.add_histogram('real_feature', self.D(self.make_input(id0, xf, volatile=True).cuda(),
...
if __name__ == '__main__':
    ...
    for i in range(len(features)):
        dict[i] = list(G.neighbors(i))
    dataset = FeatureGraphDataset(features, labels, dict)
    dataset.read_embbedings(embbeding_file='emb_file')
    dataset.setting(label_num_per_class=20, test_num=1000)

    gan = GraphSGAN(Generator(200, dataset.k + dataset.d), Discriminator(dataset.k + dataset.d, dataset.m), dataset,
                    args)
    gan.train()
